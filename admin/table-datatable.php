...
$msg = '';
// Các xử lý trạng thái
if (isset($_GET['delete'])) {
    $del_id = (int) $_GET['delete'];
    $stmt = $conn->prepare('DELETE FROM motel WHERE ID = ?');
    $stmt->bind_param('i', $del_id);
    if ($stmt->execute()) $msg = 'Đã xóa phòng trọ.';
}
    ...
    <tr>
        ...
        <td>
            ...
            <?php if ((int) $row['approve'] === 0): ?>
                <a href="?approve=<?php echo (int) $row['ID']; ?>" class="btn btn-sm btn-success" onclick="return confirm('Duyệt?')">Duyệt</a>
                <a href="?reject=<?php echo (int) $row['ID']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Từ chối?')">Từ chối</a>
            <?php endif; ?>

            <?php if ((int) $row['approve'] === 1): ?>
                <a href="?hide=<?php echo (int) $row['ID']; ?>" class="btn btn-sm btn-secondary" onclick="return confirm('Ẩn?')">Ẩn</a>
            <?php endif; ?>

            <?php if ((int) $row['approve'] === 2 || (int) $row['approve'] === 3): ?>
                <a href="?show=<?php echo (int) $row['ID']; ?>" class="btn btn-sm btn-success">Hiện</a>
            <?php endif; ?>

            <a href="?delete=<?php echo (int) $row['ID']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Xóa?')">Xóa</a>
        </td>
    </tr>
<?php endwhile; ?>
</tbody>